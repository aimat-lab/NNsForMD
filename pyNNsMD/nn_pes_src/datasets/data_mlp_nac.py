...
def mlp_nac_make_random_shuffle(datalist,shuffle_ind=None):
    """
...
def mlp_nac_save_data_to_folder(x,y,target_model,mod_dir,random_shuffle):
    ...
    if(random_shuffle == False):
        ...
        if(isinstance(y,list)):
            ...
            shuffle_list = [x] + [y]
        #Make random shuffle
        ind_shuffle, datalist  = mlp_nac_make_random_shuffle(shuffle_list)
        x_out = datalist[0]
        if(len(datalist)>2):
            y_out = datalist[1:] 
        else:
...
def mlp_nac_merge_data_in_chunks(data1,data2,split_size):
    """
