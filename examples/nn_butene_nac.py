...
from pyNNsMD.hypers.hyper_mlp_nac import DEFAULT_HYPER_PARAM_NAC as hyper
...
nn.data(atoms=atoms, geometries=geos, couplings=nac)

nn.train_test_split(dataset_size=len(energy), n_splits=5)
nn.training([hyper["training"]]*2, fit_mode="training")
# If GPU available.
fit_error = nn.fit(["training_mlp_nac"]*2, fit_mode="training", gpu_dist=[0, 0], proc_async=True)
print(fit_error)
